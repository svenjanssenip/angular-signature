...
angular.module("signature").directive("signaturePad", [
  ...
  function ($interval, $timeout, $window) {
    ...
    return {
      ...
      replace: true,
      template:
        '<div class="signature" style="width: 100%; max-width:{{width}}px; height: 100%; max-height:{{height}}px;"><canvas style="display: block; margin: 0 auto;" ng-mouseup="onMouseup()" ng-mouseleave="onMouseleave($event)" ng-mousedown="notifyDrawing({ drawing: true })"></canvas></div>',
      scope: {
        accept: "=?",
        clear: "=?",
        disabled: "=?",
